...
// 策略模式
const strategy = {
    isRole: function (value, error) {
      if(value !== 'juejin') {
        return error
      }
    },
    isGrade: function (value, error) {
        console.log(value, error)
      if(value < 1) {
        return error
      }
    },
    isJob: function (value, error) {
      if(value !== 'FE') {
        return error
      }
    },
    isType: function (value, error) {
      if(value !== 'eat melons') {
        return error
      }
    }
  ...
  Validator.prototype.add = function (value, rule, error) {
    if(strategy[rule]) {
     function _run() {
        return strategy[rule].call(null,value,error)
     }
      this.cache.push(_run)
    }
  }
  Validator.prototype.start = function (){
  ...
  function checkAuth(data) {
    ...
    validator.add(data.role, 'isRole', 'isRole is invalid')
    validator.add(data.grade, 'isGrade', 'isGrade is invalid')
    validator.add(data.job, 'isJob', 'isJob is invalid')
    validator.add(data.type, 'isType', 'isType is invalid')
